 ...
 * $Id: DOFrame.cpp 22292 2016-04-28 10:31:04Z ritt $
 */
...
char svn_revision[] = "$Id: DOFrame.cpp 22292 2016-04-28 10:31:04Z ritt $";

char drsosc_version[] = "5.0.5";
...
void DOFrame::LoadConfig(char *error, int size)
{
   ...
   if (fh < 0) {
      ...
      wxMessageBox((const wxChar*)wxString::FromAscii(str), wxT("DRSOsc"), wxOK | wxICON_ERROR);
      return;
   }
   close(fh);
...
void DOFrame::OnTimer(wxTimerEvent& event)
{
   ...
   if (m_first) {
      ...
      if (m_osci->GetNumberOfBoards() && 
         ...
         wxMessageBox((const wxChar*)wxString::FromAscii(str), wxT("DRS Oscilloscope Warning"), wxOK | wxICON_EXCLAMATION, this);
      }
